<div align="center">

<!-- Typing SVG Header -->
<a href="https://github.com/amitpadhan525">
  <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&weight=700&size=28&duration=2500&pause=1000&color=00F5FF&center=true&vcenter=true&width=650&lines=AMIT+PADHAN;B.Tech+Cybersecurity+Student;Offensive+Security+%26+Red+Teaming;Arch+Linux+%2B+Hyprland+Enthusiast" alt="Typing Header" />
</a>

<p align="center">
  <b>Offensive Security | Linux System Internals | Arch Linux Specialist 🐧 | Red Teaming</b>
</p>

<!-- Mobile App Skill Icons (Social & Connect) -->
<p align="center">
  <a href="https://github.com/amitpadhan525">
    <img src="https://skillicons.dev/icons?i=github,linkedin,instagram,discord,gmail&theme=dark" alt="Social Mobile Icons" />
  </a>
</p>

<!-- Quick Link Badges -->
<p align="center">
  <a href="https://amitpadhan525.github.io/"><img src="https://img.shields.io/badge/Portfolio-00F5FF?style=flat&logo=googlechrome&logoColor=black" alt="Portfolio"></a>
  <a href="https://tryhackme.com/p/amitpadhan525"><img src="https://img.shields.io/badge/TryHackMe-212C42?style=flat&logo=tryhackme&logoColor=white" alt="TryHackMe"></a>
  <a href="https://profile.hackthebox.com/profile/019c3941-83d7-70be-9be8-8f006c7a93f9"><img src="https://img.shields.io/badge/HackTheBox-9FEF00?style=flat&logo=hackthebox&logoColor=black" alt="HackTheBox"></a>
</p>

</div>

---

### ⚡ System Overview (`neofetch --cyber`)

```bash
       /\         OS: Arch Linux x86_64 🐧
      /  \        WM / Environment: Hyprland (Wayland) + Waybar + Rofi
     /\   \       Terminal: Kitty (JetBrainsMono Nerd Font)
    /      \      Education: B.Tech CS / Cybersecurity Student
   /   ,,   \     Primary Focus: Offensive Security, Red Teaming, Web Security & System Internals
  /   |  |  -\    Virtual Lab: QEMU/KVM (br0 bridge, tap0-3, isolated 192.168.100.x subnet)
 /_-''    ''-_\   Key Projects: linux-eye (monitoring), ReconX (recon), qemu-kvm-lab (virtualization)
```

---

### 📱 Tech Stack & Environments (Mobile Icon Style)

<div align="center">

<p align="center">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=py,bash,c,cpp,arch,kali,ubuntu,linux,git,github,docker,vscode,powershell,vim&theme=dark&perline=7" alt="Mobile App Skill Icons Grid" />
  </a>
</p>

</div>

#### 🛡️ Pentesting & Security Toolkit

<p align="center">
  <img src="https://img.shields.io/badge/Nmap-00F5FF?style=flat&logo=nmap&logoColor=black" alt="Nmap">
  <img src="https://img.shields.io/badge/Burp_Suite-FF6600?style=flat&logo=burpsuite&logoColor=white" alt="Burp Suite">
  <img src="https://img.shields.io/badge/Wireshark-1679A7?style=flat&logo=wireshark&logoColor=white" alt="Wireshark">
  <img src="https://img.shields.io/badge/Metasploit-000000?style=flat&logo=metasploit&logoColor=white" alt="Metasploit">
  <img src="https://img.shields.io/badge/Gobuster-00DAE4?style=flat&logo=go&logoColor=black" alt="Gobuster">
  <img src="https://img.shields.io/badge/Evil--WinRM-000000?style=flat&logo=windows&logoColor=white" alt="Evil-WinRM">
  <img src="https://img.shields.io/badge/Responder-FF0055?style=flat&logo=security-checked&logoColor=white" alt="Responder">
  <img src="https://img.shields.io/badge/John_the_Ripper-555555?style=flat&logo=key&logoColor=white" alt="John the Ripper">
  <img src="https://img.shields.io/badge/Hashcat-CC0000?style=flat&logo=cpu&logoColor=white" alt="Hashcat">
  <img src="https://img.shields.io/badge/Hydra-00FF66?style=flat&logo=security-checked&logoColor=black" alt="Hydra">
</p>

---

### 🚀 Featured Projects & Infrastructure

<table>
  <tr>
    <td width="50%" valign="top">
      <h4 align="center">🛡️ <a href="https://github.com/amitpadhan525/linux-eye">linux-eye</a></h4>
      <p align="center">
        <img src="https://img.shields.io/badge/Python-Security%20Monitor-00F5FF?style=flat-square" alt="Python">
      </p>
      <p>Real-time Linux host security monitoring system. Tracks process creations, kernel signals, event logs, and suspicious activity using <code>psutil</code>, <code>inotify</code>, and multi-threading.</p>
    </td>
    <td width="50%" valign="top">
      <h4 align="center">🖥️ <a href="https://github.com/amitpadhan525/qemu-kvm-lab">qemu-kvm-lab</a></h4>
      <p align="center">
        <img src="https://img.shields.io/badge/Bash-Virtualization%20Lab-00FF66?style=flat-square" alt="Bash">
      </p>
      <p>Automated script setting up isolated security lab networks on Arch Linux with bridge (<code>br0</code>), TAP interfaces, Kali Linux, Win 11, Metasploitable2, TPM 2.0 & VirtIO acceleration.</p>
    </td>
  </tr>
  <tr>
    <td width="50%" valign="top">
      <h4 align="center">🔍 <a href="https://github.com/amitpadhan525/ReconX">ReconX</a></h4>
      <p align="center">
        <img src="https://img.shields.io/badge/Automation-Reconnaissance-FF6600?style=flat-square" alt="Recon">
      </p>
      <p>Automated cybersecurity reconnaissance and footprinting framework for fast target mapping, subdomain discovery, and attack surface assessment.</p>
    </td>
    <td width="50%" valign="top">
      <h4 align="center">🎨 <a href="https://github.com/amitpadhan525/sddm-themes">sddm-themes</a></h4>
      <p align="center">
        <img src="https://img.shields.io/badge/UI%2FUX-Glassmorphism-violet?style=flat-square" alt="UI">
      </p>
      <p>Glassmorphism-inspired theme for SDDM featuring digital clocks, vibrant neon accents, and clean UI aesthetics for Arch Linux desktop setups.</p>
    </td>
  </tr>
  <tr>
    <td width="50%" valign="top">
      <h4 align="center">⚙️ <a href="https://github.com/amitpadhan525/linux-dotfiles">linux-dotfiles</a></h4>
      <p align="center">
        <img src="https://img.shields.io/badge/Config-Hyprland%20%2F%20Arch-1793D1?style=flat-square" alt="Dotfiles">
      </p>
      <p>Personalized Arch Linux environment configurations, Hyprland window manager scripts, Waybar widgets, Kitty terminal themes, Rofi launcher, and system tweaks.</p>
    </td>
    <td width="50%" valign="top">
      <h4 align="center">📚 <a href="https://github.com/amitpadhan525/security-notes">security-notes</a></h4>
      <p align="center">
        <img src="https://img.shields.io/badge/Docs-Writeups%20%26%20Labs-FF0055?style=flat-square" alt="Notes">
      </p>
      <p>Comprehensive repository of penetration testing writeups, TryHackMe & Hack The Box lab solutions, CTF notes, and ethical security research documentation.</p>
    </td>
  </tr>
</table>

---

> [!IMPORTANT]
> **Ethical Disclaimer:** All tools, research scripts, and lab configurations created or maintained by me are built strictly for authorized penetration testing, educational purposes, and ethical security research.

<br>
1